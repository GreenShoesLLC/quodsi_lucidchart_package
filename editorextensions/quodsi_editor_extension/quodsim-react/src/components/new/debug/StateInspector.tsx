...
type StateSection = 'auth' | 'subscription' | 'selection' | 'simulation' | 'app';
            ...
            <div className="state-header">
              ...
              {activeSection === 'auth' && state.auth.lastUpdated && (
                <span className="timestamp">
                  Updated: {new Date(state.auth.lastUpdated).toLocaleTimeString()}
                </span>
              )}
              {activeSection === 'subscription' && state.subscription.lastUpdated && (
                <span className="timestamp">
                  Updated: {new Date(state.subscription.lastUpdated).toLocaleTimeString()}
                </span>
              )}
              {activeSection === 'selection' && state.selection.lastUpdated && (
                <span className="timestamp">
                  Updated: {new Date(state.selection.lastUpdated).toLocaleTimeString()}
                </span>
              )}
              {activeSection === 'simulation' && state.simulation.lastUpdated && (
                <span className="timestamp">
                  Updated: {new Date(state.simulation.lastUpdated).toLocaleTimeString()}
                </span>
              )}
            </div>
