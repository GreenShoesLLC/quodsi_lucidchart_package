        ...
        <>
          <div className="test-messages">
            ...
            <div className="button-group">
              ...
              <button onClick={() => sendTestMessage(EnvelopeMessageType.SELECTION_CHANGED)}>
                SELECTION_CHANGED
              </button>
              <button onClick={() => sendTestMessage(EnvelopeMessageType.MODEL_RUN_REQUEST)}>
                MODEL_RUN_REQUEST
              </button>
            </div>
          </div>
            ...
            {messageLog.length === 0 ? (
              ...
              <ul>
                {messageLog.map((entry) => (
                  <li 
                    key={entry.id}
                    className={`message-entry ${entry.direction === 'in' ? 'incoming' : 'outgoing'}`}
          ...
          {selectedMessage && (
            <div className="message-details">
              ...
              <div className="detail-header">
                <span>Type: <strong>{selectedMessage.type}</strong></span>
                <span>Direction: <strong>{selectedMessage.direction === 'in' ? 'Incoming' : 'Outgoing'}</strong></span>
                <span>Time: <strong>{new Date(selectedMessage.timestamp).toLocaleTimeString()}</strong></span>
              </div>
