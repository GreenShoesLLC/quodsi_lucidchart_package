...
export const LucidApp: React.FC<LucidAppProps> = ({ panelType = 'model' }) => {
  const { auth, selection, subscription, simulation, app, sendMessage } = useMessaging();
    ...
    <div className="lucid-app">
      ...
      <div className="app-header">
        ...
        <div className="panel-info">
          <span>Panel Type: <strong>{panelType}</strong></span>
          <span>Initialized: <strong>{app.initialized ? 'Yes' : 'No'}</strong></span>
          <span>Authenticated: <strong>{auth.isAuthenticated ? 'Yes' : 'No'}</strong></span>
        </div>
      </div>
      ...
      <div className="message-status">
        <div>
          ...
          <p>Last message sent: <code>{lastMessageSent || 'None'}</code></p>
          <p>Last message received: <code>{lastMessageReceived || 'None'}</code></p>
