                  ...
                  {(['min', 'max', 'step'] as const).map((k) => (
                    <label key={k} className="flex flex-col text-xs">
                      ...
                      <input
                        ...
                        min={k === 'min' && !bounds?.minExclusive ? bounds?.min : undefined} max={k === 'max' ? bounds?.max : undefined}
                      ...
                      {(['min', 'max', 'step'] as const).map((k) => (
                        <label key={k} className="flex flex-col text-xs">
                          ...
                          <input
                            ...
                            min={k === 'min' && !durationBounds?.minExclusive ? durationBounds?.min : undefined} max={k === 'max' ? durationBounds?.max : undefined}
                            className="border rounded px-2 py-1 text-sm w-16"
                            value={lever.range?.[k] ?? ''}
                      ...
                      {(['min', 'max', 'step'] as const).map((k) => (
                        <label key={k} className="flex flex-col text-xs">
                          ...
                          <input
                            ...
                            min={k === 'min' && !priorityBounds?.minExclusive ? priorityBounds?.min : undefined} max={k === 'max' ? priorityBounds?.max : undefined}
                            className="border rounded px-2 py-1 text-sm w-16"
                            value={lever.range?.[k] ?? ''}
