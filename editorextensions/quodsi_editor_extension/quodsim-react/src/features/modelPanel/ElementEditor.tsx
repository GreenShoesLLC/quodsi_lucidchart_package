...
import { SimulationObjectType, EditorReferenceData, DiagramElementType } from '@quodsi/shared';
...
export const ElementEditor: React.FC<ElementEditorProps> = ({
  ...
  const getElementTypeDisplay = () => {
    const displayText = elementType === SimulationObjectType.Model
      ? "Model Properties"
      : elementType
      ? `Edit ${elementType}`
      : "Element Editor";
  ...
  const renderEditor = () => {
    console.log(`[ElementEditor] Rendering editor for element type: ${elementType}, currentElement:`, currentElement);
    ...
    console.log('[ElementEditor] EDITOR SELECTION - Detailed type info:', {
      ...
      isElementTypeSimObjType: Object.values(SimulationObjectType).includes(elementType as SimulationObjectType),
    ...
    if (elementType === SimulationObjectType.Resource || elementType === 'Resource') {
    ...
    } else if (currentElement?.metadata?.type === SimulationObjectType.Resource || 
               currentElement?.metadata?.type === 'Resource') {
      console.log('[ElementEditor] Resource detected from currentElement.metadata.type');
      isResource = true;
      resourceDetectionSource = 'currentElement.metadata.type';
    } else if (currentElement?.q_meta?.type === 'Resource') {
      console.log('[ElementEditor] Resource detected from currentElement.q_meta.type');
      isResource = true;
    ...
    console.log('[ElementEditor] Resource detection result:', { isResource, resourceDetectionSource });
    
    // Additional safety to ensure we never have block/line as element types
    let safeElementType = isResource ? SimulationObjectType.Resource : elementType;
    
    // Check if q_meta or metadata is available for this element
    const metadataType = !isResource && (currentElement?.metadata?.type || currentElement?.q_meta?.type);
    
    if (metadataType) {
      console.log('[ElementEditor] Using type from metadata/q_meta:', metadataType);
    ...
    // Only if we don't have metadata type and not a resource, then use element type fallbacks
    else if (!isResource && (elementType === DiagramElementType.BLOCK || elementType === 'block') && currentElement) {
      console.warn('[ElementEditor] Block type without metadata classification', {
        currentElementId: currentElement?.id,
    ...
    } else if (!isResource && (elementType === DiagramElementType.LINE || elementType === 'line') && currentElement) {
      ...
      if (currentElement && !currentElement.metadata) {
        ...
        console.log('[ElementEditor] Updated metadata type from', oldType, 'to', SimulationObjectType.Connector);
      }
    }
    ...
    if (safeElementType === "Model" || safeElementType === SimulationObjectType.Model) {
    ...
    if (!safeElementData?.id && elementType) {
      ...
      return (
        <div className="p-4 text-red-600 bg-red-50 border border-red-200 rounded-md shadow-sm">
          ...
          <div className="text-sm text-red-500">Element ID missing or data incomplete</div>
        </div>
      );
    }
    ...
    if (isResource) {
      console.log('[ElementEditor] Rendering ResourceEditor because isResource flag is true', {
    ...
    switch (safeElementType) {
      ...
      case DiagramElementType.BLOCK: // Handle when elementType is directly passed as block
        console.log('[ElementEditor] Rendering ActivityEditor for type:', safeElementType, {
          elementId: safeElementData?.id,
      ...
      case 'Generator':
        console.log('[ElementEditor] Rendering GeneratorEditor for type:', safeElementType);
        return (
          <GeneratorEditor
      ...
      case 'Resource':
        console.log('[ElementEditor] Rendering ResourceEditor for type:', safeElementType);
        return (
          <ResourceEditor
            resource={safeElementData}
            onSave={onSave}
      ...
      case 'Entity':
        console.log('[ElementEditor] Rendering EntityEditor for type:', safeElementType);
        return (
          <EntityEditor
      ...
      case DiagramElementType.LINE: // Handle when elementType is directly passed as line
        console.log('[ElementEditor] Rendering ConnectorEditor for type:', safeElementType);
        return (
          <ConnectorEditor
      ...
      default:
        console.warn('[ElementEditor] No matching editor found for element type:', safeElementType, {
          elementId: safeElementData?.id,
