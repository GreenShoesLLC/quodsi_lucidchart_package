// handlers/ResourceCrossRepTableHandler.ts
...
/**
 * Handler for creating resource cross replication tables
 */
export class ResourceCrossRepTableHandler extends BaseTableHandler {
    /**
    ...
    getDefaultTitle(): string {
        return 'Resource Cross Replication';
    }
    ...
    async canCreateTable(): Promise<boolean> {
        const data = await this.resultsReader.getResourceCrossRepData();
    ...
    async createTable(page: PageProxy, position: { x: number, y: number }): Promise<TableCreationResult> {
        ...
        try {
            ...
            const generator = this.tableGenerator.factory.getGenerator('resourceCrossRep');
