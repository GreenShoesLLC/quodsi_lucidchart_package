// handlers/EntityCrossRepTableHandler.ts
...
/**
 * Handler for creating entity cross replication tables
 */
export class EntityCrossRepTableHandler extends BaseTableHandler {
    /**
    ...
    getDefaultTitle(): string {
        return 'Entity Cross Replication';
    }
    ...
    async canCreateTable(): Promise<boolean> {
        const data = await this.resultsReader.getEntityCrossRepData();
    ...
    async createTable(page: PageProxy, position: { x: number, y: number }): Promise<TableCreationResult> {
        ...
        try {
            ...
            const generator = this.tableGenerator.factory.getGenerator('entityCrossRep');
