// actions/checkScenarioTaskStatusAction.ts
...
interface CheckScenarioTaskStatusRequest {
    documentId: string;
    scenarioId: string;
}

interface CheckScenarioTaskStatusResult {
    success: boolean;
    state: string;
    failed: boolean;
    scenarioName?: string;
 ...
 */
export const checkScenarioTaskStatusAction = async (
    action: DataConnectorAsynchronousAction
): Promise<CheckScenarioTaskStatusResult> => {
    const config = getConfig();
    const loggingLevel = config.logging?.hardRefreshActionLoggingLevel || LoggingLevel.NORMAL;
    const logger = new ActionLogger('[CheckScenarioTaskStatusAction]', loggingLevel);

    try {
        logger.important('Starting scenario task status check');

        const data = action.data as CheckScenarioTaskStatusRequest;
        ...
        logger.error(`Error checking scenario task status: ${errorMessage}`);
