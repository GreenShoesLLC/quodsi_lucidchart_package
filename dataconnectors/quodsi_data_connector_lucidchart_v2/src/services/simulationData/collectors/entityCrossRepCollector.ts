...
export async function fetchEntityCrossRep(
    ...
    try {
        ...
        const mappedResult: EntityCrossRepSummaryData[] = result.map((item: any) => ({
            ...
            // WIP statistics
            min_wip_mean: item.min_wip_mean,
            min_wip_std_dev: item.min_wip_std,
            min_wip_min: item.min_wip_min,
            min_wip_max: item.min_wip_max,

            max_wip_mean: item.max_wip_mean,
            max_wip_std_dev: item.max_wip_std,
            max_wip_min: item.max_wip_min,
            max_wip_max: item.max_wip_max,
        ...
        const validatedResult = mappedResult.map(item => {
            const validItem: EntityCrossRepSummaryData = {
                ...
                // WIP statistics
                min_wip_mean: item.min_wip_mean ?? 0,
                min_wip_std_dev: item.min_wip_std_dev ?? 0,
                min_wip_min: item.min_wip_min ?? 0,
                min_wip_max: item.min_wip_max ?? 0,

                max_wip_mean: item.max_wip_mean ?? 0,
                max_wip_std_dev: item.max_wip_std_dev ?? 0,
                max_wip_min: item.max_wip_min ?? 0,
                max_wip_max: item.max_wip_max ?? 0,
